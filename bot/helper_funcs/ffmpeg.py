...
async def convert_video(video_file, output_directory, total_time, bot, message, chan_msg):
    ...
    aa = kk.split(".")[-1]
    out_put_file_name = kk.replace(f".{aa}", "[@OM_links].mkv")
    #out_put_file_name = video_file + "_compressed" + ".mkv"
    progress = output_directory + "/" + "progress.txt"
    with open(progress, 'w') as f:
      pass
    ##  -metadata title='Anime Sensei [Join t.me/OM_links]' -vf drawtext=fontfile=Italic.ttf:fontsize=20:fontcolor=black:x=15:y=15:text='Anime Sensei'
